...
sudo apt install -y git mpv neovim neofetch sox uwufetch vscode jdk17-openjdk gcc tmux unzip zip curl nodejs npm zsh
...
then
	...
	cp -r ./terminals/nvim $nVim 
else
	echo "$nVim directory exists."
	echo "Copying"
	cp -r -f ./terminals/nvim /home/$user/.config
fi
